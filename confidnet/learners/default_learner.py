...
class DefaultLearner(AbstractLeaner):
    def train(self, epoch):
        ...
        logs_dict = OrderedDict({
            ...
            "lr": {"value": self.optimizer.param_groups[0]["lr"], "string": f"{self.optimizer.param_groups[0]['lr']:05.1e}"},
            "train/loss_nll": {"value": loss / len_data, "string": f"{(loss / len_data):05.4e}"}
        })
        for s in scores:
        ...
        if self.val_loader is not None:
            ...
            logs_dict["val/loss_nll"] = {"value": val_losses["loss_nll"].item() / self.nsamples_val, "string": f"{(val_losses['loss_nll'].item() / self.nsamples_val):05.4e}"}
        ...
        logs_dict["test/loss_nll"] = {"value": test_losses["loss_nll"].item() / self.nsamples_test, "string": f"{(test_losses['loss_nll'].item() / self.nsamples_test):05.4e}"}
    ...
    def evaluate(self, dloader, len_dataset, split="test", mode="mcp", samples=50, verbose=False):
      ...
      loss = 0
      pred = None  # Ensure pred is always defined


      if mode not in ["mcp", "tcp", "mc_dropout", "confidnet"]:
      ...
      for batch_id, (data, target) in enumerate(loop):
          ...
          with torch.no_grad():
              if mode == "mcp":
                  ...
                  labels_hot = misc.one_hot_embedding(target, self.num_classes).to(self.device)
